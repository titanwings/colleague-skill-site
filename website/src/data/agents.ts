 ...
 *
 * Mirrors the "installation options" pattern used by archify
 * (https://github.com/tt-a1i/archify#installation-options): every host gets an
 * exact command, an exact install location, and an honest capability note, so a
 * visitor never has to guess whether their agent is supported.
 *
