...
| `website/tailwind.config.mjs`, `website/src/styles/global.css` | **Contrast fix**: new `--accent-ink` token (white in light, near-black in dark) for text on accent fills; light-theme accent darkened `#e14a0e → #c73e0c`; dark-theme `--accent-soft` lightened; legacy `surface-300…500` aliases now resolve to `--ink-muted` instead of the dim decorative tone; `surface-600/700` stay as borders. `.btn-primary` uses `text-accent-ink` instead of `text-white`. |
...
| axe-core `serious`/`critical` violations (home/gallery/detail × light/dark) | **24 failing page-theme combos reported, 313 `color-contrast` nodes** | 6 combos, **94 nodes** (see below) |
